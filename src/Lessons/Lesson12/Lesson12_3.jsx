...
// Image Assets
import lessonKeyboard from "../../assets/lesson12.3.png";

// Comprehensive hand territories for the ENTIRE KEYBOARD
const leftHandKeys = ["q", "w", "e", "r", "t", "a", "s", "d", "f", "g", "z", "x", "c", "v", "b", "Q", "W", "E", "R", "T", "A", "S", "D", "F", "G", "Z", "X", "C", "V", "B"];
const rightHandKeys = ["y", "u", "i", "o", "p", "h", "j", "k", "l", ";", "'", "n", "m", ",", ".", "/", "Y", "U", "I", "O", "P", "H", "J", "K", "L", ":", "\"", "N", "M", "<", ">", "?"];

// Sequential paragraphs featuring full keyboard pangrams and Z/X combinations
const paragraphLessons = [
  {
    id: 1,
    title: "Classic Pangrams",
    text: "The quick brown fox jumps over the lazy dog. Pack my box with five dozen liquor jugs. Zack, stop exploring very quickly!",
    focus: "Full Alphabet Coverage, Z & X stretches",
  },
  {
    id: 2,
    title: "Complex Punctuation & Caps",
    text: "Crazy Frederick bought many very exquisite opal jewels. How exactly do you mix crazy flavors? Just squeeze a bit of lemon juice.",
    focus: "Shift Keys, Question Marks (?), Quotes (\")",
  },
  {
    id: 3,
    title: "The Ultimate Mastery Sprint",
    text: "We have finally unlocked the whole keyboard! Practice makes perfect, right? Zero boxes are left in the maze. She wants to maximize her next prize.",
    focus: "Sustained Rhythm, Exclamation Marks (!), Flow",
  }
];

export default function Lesson12_3() {
  ...
  const handleKeyDown = (e) => {
    ...
    if (key.length === 1 || key === " ") {
      ...
      if (isCorrect) {
        ...
        if (expectedChar.toLowerCase() === "z") {
          displayInstruction = "DROP LEFT PINKY DOWN-RIGHT FOR Z";
        } else if (expectedChar.toLowerCase() === "x") {
          displayInstruction = "DROP LEFT RING FINGER DOWN-RIGHT FOR X";
        } else if (expectedChar === "/") {
          displayInstruction = "STRETCH RIGHT PINKY DOWN-RIGHT FOR SLASH (/)";
        } else if (expectedChar === "?") {
          displayInstruction = "HOLD LEFT SHIFT + PRESS SLASH (/)";
        } else if (expectedChar === "\"") {
          displayInstruction = "HOLD LEFT SHIFT + PRESS APOSTROPHE";
        } else if (expectedChar === "!") {
          displayInstruction = "HOLD RIGHT SHIFT + PRESS 1";
        } else if (expectedChar === ":") {
          displayInstruction = "HOLD LEFT SHIFT + PRESS SEMICOLON";
        } else if (expectedChar === " ") {
          displayInstruction = "STRIKE SPACEBAR WITH THUMB";
        } else if (expectedChar === expectedChar.toUpperCase() && expectedChar !== " " && expectedChar.toLowerCase() !== expectedChar.toUpperCase()) {
          displayInstruction = "HOLD SHIFT KEY FOR CAPITAL LETTERS";
        } else {
          displayInstruction = `INCORRECT KEY. TARGET WAS '${expectedChar}'`;
        }
  ...
  const handleNextLevel = () => {
    if (currentLevel < paragraphLessons.length - 1) {
      ...
      navigate("/lessons"); // Finished all 3 paragraphs
    }
  };
  ...
  const dynamicShiftTarget = nextTargetChar !== "—" && nextTargetChar !== " " 
    ? (nextTargetChar === "\"" || nextTargetChar === ":" || nextTargetChar === "?" || nextTargetChar === "!" || (nextTargetChar === nextTargetChar.toUpperCase() && nextTargetChar !== ";" && nextTargetChar !== "," && nextTargetChar !== "'" && nextTargetChar !== "." && nextTargetChar !== "/"))
    : false;
  ...
  return (
    ...
    >
      ...
      <h1 className="text-4xl font-extrabold tracking-wide text-center w-full max-w-[1440px] mx-auto mb-12 uppercase">
        STAGE 12.3 — {paragraphLessons[currentLevel].title}
      </h1>
        ...
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 backdrop-blur-md p-4">
          <div className="bg-[#02040f] border-2 border-[#2b3245] rounded-[48px] p-16 flex flex-col items-center shadow-2xl text-center max-w-[640px] w-full mx-auto transition-all">
            ...
            <h2 className="text-5xl font-black text-purple-400 mb-8 tracking-wide">
              {currentLevel < paragraphLessons.length - 1 ? "Paragraph Cleared! 🚀" : "Course Completed! 🏆"}
            </h2>
            {currentLevel === paragraphLessons.length - 1 && (
              <p className="text-gray-300 font-medium mb-4">You have successfully mastered every letter key on the standard keyboard layout.</p>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-[500px]">
              ...
              >
                {currentLevel < paragraphLessons.length - 1 ? "Next Paragraph" : "Back to Stages"}
              </button>
