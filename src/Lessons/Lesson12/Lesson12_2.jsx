...
// Image Assets
import lessonKeyboard from "../../assets/lesson12.1.png";

// Comprehensive hand territories for the ENTIRE alphabet (Added Z and X)
const leftHandKeys = ["a", "s", "d", "f", "g", "c", "v", "w", "e", "r", "t", "q", "b", "z", "x", "A", "S", "D", "F", "G", "C", "V", "W", "E", "R", "T", "Q", "B", "Z", "X"];
const rightHandKeys = ["h", "j", "k", "l", ";", "'", "n", "m", "u", "i", "o", "p", "y", ",", ".", ":", "\"", "?", "/", "H", "J", "K", "L", "N", "M", "U", "I", "O", "P", "Y"];

// Generates full paragraphs with a heavy focus on Z and X
const generateWordParagraph = () => {
  const templates = [
    "The crazy fox jumps over the lazy dog. Max played the xylophone in the jazz band.",
    "Six fuzzy zebras relax by the zoo. Buzzing bees mix with the breeze.",
    "Max was amazed by the size of the box. Exact prices are fixed in this zone.",
    "Did you examine the bronze zipper? The text is a mix of crazy words.",
    "Zero boxes are left in the maze. She wants to maximize her next prize."
  ];
...
export default function Lesson12_2() {
  ...
  const handleKeyDown = (e) => {
    if (showPopup) {
      if (e.key === " ") {
        e.preventDefault();
        navigate("/lessons"); // You can route this to 12.3 or the next stage if you have one
      }
      return;
    }
  ...
  return (
    ...
    >
      ...
      <h1 className="text-4xl font-extrabold tracking-wide text-center w-full max-w-[1440px] mx-auto mb-12">
        STAGE 12.2 — Word Integration (Z and X Keys)
      </h1>
        ...
        <div className="flex flex-col gap-10 w-full">
          ...
          <div className="bg-[#111827] border border-[#2b3245] border-l-[10px] border-l-purple-500 rounded-2xl px-10 py-8 shadow-2xl flex flex-col w-full">
            ...
            <p className="text-md leading-relaxed text-gray-300 font-medium">
              You are now integrating <span className="text-amber-400 font-bold">Z</span> and <span className="text-amber-400 font-bold">X</span> into full sentences alongside every other key on the board! Be mindful of your left pinky and left ring finger, as these are the hardest bottom-row stretches.
            </p>
          </div>
        ...
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 backdrop-blur-md p-4">
          <div className="bg-[#02040f] border-2 border-[#2b3245] rounded-[48px] p-16 flex flex-col items-center shadow-2xl text-center max-w-[640px] w-full mx-auto transition-all">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-[500px]">
              <button
                onClick={() => navigate("/Lesson12/12.3")}
