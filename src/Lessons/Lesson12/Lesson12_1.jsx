...
// Hand territories for tracking targets (Both Z and X are left hand!)
const leftHandKeys = ["z", "x"];
const rightHandKeys = [];

// Generates practice streams containing ONLY 'z', 'x', and spaces
const generateWordParagraph = () => {
  const templates = [
    "z x z x z x x z z x z x z z x z x z x x z z x z x x z x z x z",
    "x z x z x x z z x z x z x x z z x z x z x z z x x z x z x x z",
    "z z x x z x z x x z z x z z x x z x z x x z z x z x z x x z z",
    "x x z z x z x z z x x z x x z z x z x z z x x z x z x z z x x",
    "z x x z z x x z z x x z z x x z z x x z z x x z z x x z x x z z",
    "x z z x x z z x x z z x x z z x x z z x x z z x x z z x x z z x"
  ];
...
export default function Lesson12_1() {
  ...
  return (
    ...
    >
      ...
      <h1 className="text-4xl font-extrabold tracking-wide text-center w-full max-w-[1440px] mx-auto mb-12">
        Lesson 12.1 — Isolated Finger Drill (Z and X Keys)
      </h1>
          ...
          <div className="bg-[#111827] border border-[#2b3245] border-l-[10px] border-l-purple-500 rounded-2xl px-10 py-8 shadow-2xl flex flex-col w-full">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-xl font-extrabold tracking-wide text-purple-400">
                Fingering Guide for Z & X
              </h2>
            </div>
            <p className="text-md leading-relaxed text-gray-300 font-medium">
              Drop your <span className="text-amber-400 font-bold">Left Pinky Finger</span> down from <span className="underline">A</span> to hit <span className="text-amber-400 font-bold">Z</span>. Drop your <span className="text-amber-400 font-bold">Left Ring Finger</span> down from <span className="underline">S</span> to hit <span className="text-amber-400 font-bold">X</span>. Both of these targets are handled exclusively by your left hand.
            </p>
          </div>
        ...
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 backdrop-blur-md p-4">
          <div className="bg-[#02040f] border-2 border-[#2b3245] rounded-[48px] p-16 flex flex-col items-center shadow-2xl text-center max-w-[640px] w-full mx-auto transition-all">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-[500px]">
              <button
                onClick={() => navigate("/lesson12/12.2")}
