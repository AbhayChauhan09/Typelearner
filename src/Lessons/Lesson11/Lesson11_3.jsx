...
// Comprehensive hand territories for all keys learned so far (Everything except Z and X)
const leftHandKeys = ["a", "s", "d", "f", "g", "c", "v", "b", "w", "e", "r", "t", "q", "A", "S", "D", "F", "G", "C", "V", "B", "W", "E", "R", "T", "Q"];
const rightHandKeys = ["h", "j", "k", "l", ";", "'", "n", "m", "u", "i", "o", "p", "y", ",", ".", "/", ":", "\"", "?", "H", "J", "K", "L", "N", "M", "U", "I", "O", "P", "Y"];

// Generates paragraphs using ALL taught keys, plus Shift symbols, but NO 'x' or 'z'
const generateWordParagraph = () => {
  const templates = [
    "Why did the brave boy quit playing? He bought a very big yellow boat.",
    "Please pack my bag with red pens, blue paper, and a quick map.",
    "Young children love to jump very quickly. Can you bring them back home?",
    "\"Watch out for the heavy rain,\" she said. The bright yellow sun came up.",
    "We might buy more fresh fruit today: apples, grapes, and very juicy plums."
  ];
...
export default function Lesson11_3() {
  ...
  const handleKeyDown = (e) => {
    if (showPopup) {
      if (e.key === " ") {
        e.preventDefault();
        navigate("/lessons"); // End of Lesson 11, return to dashboard
      }
      return;
    }
  ...
  const dynamicShiftTarget = nextTargetChar !== "—" && nextTargetChar !== " " 
    ? (nextTargetChar === "\"" || nextTargetChar === ":" || nextTargetChar === "?" || (nextTargetChar === nextTargetChar.toUpperCase() && nextTargetChar !== ";" && nextTargetChar !== "," && nextTargetChar !== "'" && nextTargetChar !== "." && nextTargetChar !== "/"))
    : false;
  ...
  return (
    ...
    >
      ...
      <h1 className="text-4xl font-extrabold tracking-wide text-center w-full max-w-[1440px] mx-auto mb-12">
        STAGE 11.3 — Full Keyboard Integration & Shift Keys
      </h1>
        ...
        <div className="flex flex-col gap-10 w-full">
          ...
          <div className="bg-[#111827] border border-[#2b3245] border-l-[10px] border-l-purple-500 rounded-2xl px-10 py-8 shadow-2xl flex flex-col w-full">
            ...
            <p className="text-md leading-relaxed text-gray-300 font-medium">
              This exercise combines almost every letter of the alphabet! Focus entirely on your <span className="text-amber-400 font-bold">Shift</span> key coordination. Remember: if the capital letter is on the right side, hold Shift with your left pinky. If it's on the left side, hold Shift with your right pinky!
            </p>
          </div>
