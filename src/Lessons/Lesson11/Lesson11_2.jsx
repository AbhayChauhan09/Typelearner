...
// Image Assets
import lessonKeyboard from "../../assets/lesson11.3.png";

// Comprehensive hand territories for all keys learned so far (Added B and Y)
const leftHandKeys = ["a", "s", "d", "f", "g", "c", "v", "w", "e", "r", "t", "q", "b", "A", "S", "D", "F", "G", "C", "V", "W", "E", "R", "T", "Q", "B"];
const rightHandKeys = ["h", "j", "k", "l", ";", "'", "n", "m", "u", "i", "o", "p", "y", ",", ".", ":", "\"", "?", "/", "H", "J", "K", "L", "N", "M", "U", "I", "O", "P", "Y"];

// Generates paragraphs using ONLY taught keys (no x, z)
const generateWordParagraph = () => {
  const templates = [
    "The boy bought a big yellow boat. Yes, they have a baby bird by the tree. Bobby says you may buy it.",
    "My buddy will bring the yellow bike today. Maybe you can carry the heavy bag. They play baseball every day.",
    "Why did the baby cry yesterday? Young boys and girls play by the bay. We buy bread and butter for everybody.",
    "Your big yellow bus is ready by the yard. Yes, the brave boy brought his dog. Anybody can try to play the piano.",
    "They buy black and brown boots. The baby monkey plays by the big tree. Yesterday, you broke the yellow toy."
  ];
...
export default function Lesson11_2() {
  ...
  const handleKeyDown = (e) => {
    if (showPopup) {
      if (e.key === " ") {
        e.preventDefault();
        navigate("/lesson11/11.3"); // Route to next appropriate lesson
      }
      return;
    }
    ...
    if (key.length === 1 || key === " ") {
      ...
      if (isCorrect) {
        ...
        if (expectedChar.toLowerCase() === "b") {
          displayInstruction = "STRETCH LEFT INDEX FINGER DOWN-RIGHT FOR B";
        } else if (expectedChar.toLowerCase() === "y") {
          displayInstruction = "STRETCH RIGHT INDEX FINGER UP-LEFT FOR Y";
  ...
  const dynamicShiftTarget = nextTargetChar !== "—" && nextTargetChar !== " " 
    ? (nextTargetChar === "\"" || nextTargetChar === ":" || nextTargetChar === "?" || nextTargetChar === nextTargetChar.toUpperCase() && nextTargetChar !== ";" && nextTargetChar !== "," && nextTargetChar !== "'" && nextTargetChar !== ".")
    : false;
  ...
  return (
    ...
    >
      ...
      <h1 className="text-4xl font-extrabold tracking-wide text-center w-full max-w-[1440px] mx-auto mb-12">
        STAGE 11.2 — Word Integration (B and Y Keys)
      </h1>
        ...
        <div className="flex flex-col gap-10 w-full">
          ...
          <div className="bg-[#111827] border border-[#2b3245] border-l-[10px] border-l-purple-500 rounded-2xl px-10 py-8 shadow-2xl flex flex-col w-full">
            ...
            <p className="text-md leading-relaxed text-gray-300 font-medium">
              We are now integrating <span className="text-amber-400 font-bold">B</span> and <span className="text-[#00E676] font-bold">Y</span> into full sentences. Pay close attention to your index fingers. Do not twist your wrist; stretch the fingers out naturally from their home keys.
            </p>
          </div>
          ...
          <div className="bg-[#02040f] border border-[#1b2236] rounded-[36px] shadow-2xl flex flex-col p-12 min-h-[320px] justify-center items-center">
            <span className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-8">
              {!startTime ? "CLICK HERE & START TYPING THE SENTENCES" : "DRILL IN PROGRESS"}
            </span>
            ...
            <div className="flex flex-wrap gap-x-1.5 gap-y-4 justify-center max-w-[850px] leading-relaxed">
              {practiceText.map((char, index) => {
                ...
                return (
                  <span 
                    key={index}
                    className={`text-3xl font-mono transition-all duration-100 font-bold ${colorClass} ${decorationClass}`}
                  >
                    {char === " " ? "␣" : char}
                  </span>
                );
        ...
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 backdrop-blur-md p-4">
          <div className="bg-[#02040f] border-2 border-[#2b3245] rounded-[48px] p-16 flex flex-col items-center shadow-2xl text-center max-w-[640px] w-full mx-auto transition-all">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-[500px]">
              <button
                onClick={() => navigate("/lesson11/11.3")}
