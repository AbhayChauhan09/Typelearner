...
// Image Assets
import lessonKeyboard from "../../assets/lesson10.1.png";

// Comprehensive hand territories for all keys learned so far (Added Q and P)
const leftHandKeys = ["a", "s", "d", "f", "g", "c", "v", "w", "e", "r", "t", "q", "A", "S", "D", "F", "G", "C", "V", "W", "E", "R", "T", "Q"];
const rightHandKeys = ["h", "j", "k", "l", ";", "'", "n", "m", "u", "i", "o", "p", ",", ".", ":", "\"", "H", "J", "K", "L", "N", "M", "U", "I", "O", "P"];

// Generates paragraphs using ONLY taught keys (no b, x, y, z)
const generateWordParagraph = () => {
  const templates = [
    "Keep quiet and put the quote up. The quick ship requires more liquid. Please quit the camp now.",
    "Quick men put the square map up. I request equal space to paint. Stop the drop and equip them.",
    "She wants to win the cup. Provide a quick quote for the part. The top step is quite steep.",
    "We hope to see the square ship. Put the pen and pan on the map. He is quite quick to request help.",
    "People want peace and quiet. The liquid drops out of the pipe. Please stop and park the car."
  ];
...
export default function Lesson10_2() {
  ...
  const handleKeyDown = (e) => {
    if (showPopup) {
      if (e.key === " ") {
        e.preventDefault();
        navigate("/lesson10/10.3"); // Adjust route for next lesson
      }
      return;
    }
    ...
    if (key.length === 1 || key === " ") {
      ...
      if (isCorrect) {
        ...
        if (expectedChar.toLowerCase() === "q") {
          displayInstruction = "STRETCH LEFT PINKY UP-LEFT FOR Q";
        } else if (expectedChar.toLowerCase() === "p") {
          displayInstruction = "STRETCH RIGHT PINKY UP-RIGHT FOR P";
  ...
  return (
    ...
    >
      ...
      <h1 className="text-4xl font-extrabold tracking-wide text-center w-full max-w-[1440px] mx-auto mb-12">
        STAGE 10.2 — Word Integration (Q and P Keys)
      </h1>
        ...
        <div className="flex flex-col gap-10 w-full">
          ...
          <div className="bg-[#111827] border border-[#2b3245] border-l-[10px] border-l-purple-500 rounded-2xl px-10 py-8 shadow-2xl flex flex-col w-full">
            ...
            <p className="text-md leading-relaxed text-gray-300 font-medium">
              We are now integrating <span className="text-amber-400 font-bold">Q</span> and <span className="text-[#00E676] font-bold">P</span> into full sentences. Pay close attention to your pinkies. They are the weakest fingers, so consciously stretch them up and outward to strike the keys accurately without moving your entire hand off the home row.
            </p>
          </div>
        ...
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 backdrop-blur-md p-4">
          <div className="bg-[#02040f] border-2 border-[#2b3245] rounded-[48px] p-16 flex flex-col items-center shadow-2xl text-center max-w-[640px] w-full mx-auto transition-all">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-[500px]">
              <button
                onClick={() => navigate("/lesson10/10.3")}
