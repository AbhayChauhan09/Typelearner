...
// Image Assets - Updated to the presumed next image sequence
import lessonKeyboard from "../../assets/lesson9.1.png";

// Hand territories for tracking targets
const leftHandKeys = ["w"];
const rightHandKeys = ["m"];

// Generates practice streams containing ONLY 'w', 'm', and spaces
const generateWordParagraph = () => {
  const templates = [
    "w m w m w m m w w m w m w w m w m w m m w w m w m m w m w m w",
    "h h g g h g h g h g h g g h g h g h g h g h h g g h g h g g h", // Kept template count structural
    "m m w w m w m w m w m w w m w m w m w m w m m w w m w m w w m",
    "w m w m w w m w m w m m w m w m w m m w m w m w m w m w m w w",
    "w m w w m m w w m w m m w m w m w m w m w m w w m m w w m m w",
    "m w m w m m w w m w m w m w w m w m m w m w m w m w w m m w w"
  ];

  // Adjusting template indices to strictly match pure w/m sequences
  const pureTemplates = [
    templates[0],
    templates[2],
    templates[3],
  ...
  const randomParagraph = pureTemplates[Math.floor(Math.random() * pureTemplates.length)];
...
export default function Lesson9_1() {
  ...
  const handleKeyDown = (e) => {
    if (showPopup) {
      if (e.key === " ") {
        e.preventDefault();
        // Assuming the next lesson is 8.3
        navigate("/Lesson9/9.2");
      }
      return;
    }
    ...
    if (key.length === 1 || key === " ") {
      ...
      if (isCorrect) {
        ...
        if (expectedChar === "w") {
          displayInstruction = "STRETCH LEFT RING FINGER UP-LEFT FOR W";
        } else if (expectedChar === "m") {
          displayInstruction = "DROP RIGHT INDEX FINGER DOWN-RIGHT FOR M";
        }
  ...
  return (
    ...
    >
      ...
      <h1 className="text-4xl font-extrabold tracking-wide text-center w-full max-w-[1440px] mx-auto mb-12">
        Lesson 9.1 — Isolated Finger Drill (w and m Keys)
      </h1>
      ...
      <div className="w-full max-w-[1440px] mx-auto grid grid-cols-1 lg:grid-cols-[200px_1fr_420px] gap-10 items-start">
        ...
        <div className="flex flex-col gap-10 w-full">
          ...
          <div className="bg-[#111827] border border-[#2b3245] border-l-[10px] border-l-purple-500 rounded-2xl px-10 py-8 shadow-2xl flex flex-col w-full">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-xl font-extrabold tracking-wide text-purple-400">
                Fingering Guide for W & M
              </h2>
            </div>
            <p className="text-md leading-relaxed text-gray-300 font-medium">
              Stretch your <span className="text-amber-400 font-bold">Left Ring Finger</span> up and slightly left from <span className="underline">S</span> to hit <span className="text-amber-400 font-bold">W</span>. Drop your <span className="text-[#00E676] font-bold">Right Index Finger</span> down and slightly right from <span className="underline">J</span> to hit <span className="text-[#00E676] font-bold">M</span>. Keep your wrists hovering naturally.
            </p>
          </div>
          ...
          {/* Dynamic Shift Indicator Panels */}
          <div className="w-full flex justify-between gap-6 bg-[#111827]/10 p-6 rounded-3xl border border-[#2b3245]/20 opacity-40 select-none pointer-events-none">
            <div className="px-6 py-3 rounded-xl border text-xs font-mono font-bold text-center w-1/2 bg-[#161f33]/40 border-gray-800 text-gray-600">
              Left Shift Panel (Inactive for this drill)
            </div>
            <div className="px-6 py-3 rounded-xl border text-xs font-mono font-bold text-center w-1/2 bg-[#161f33]/40 border-gray-800 text-gray-600">
              Right Shift Panel (Inactive for this drill)
            </div>
          </div>
        ...
        <div className="flex flex-col gap-6 w-full">
          ...
          <div className="bg-[#111827] border border-[#2b3245] rounded-3xl p-8 shadow-xl flex flex-col items-center text-center justify-center min-h-[180px]">
            ...
            <h3 className={`text-6xl font-black font-mono ${nextTargetChar === " " ? "text-gray-500" : nextTargetChar === "w" ? "text-amber-400" : "text-[#00E676]"}`}>
