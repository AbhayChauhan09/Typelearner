...
// Practice words updated to lowercase to allow fluid lowercase typing without triggering errors
const practiceWords = [
  "ask", "dad", "fall", "sail", "desk", "jail", "kids", "lake", "idea", "safe",
  "like", "fade", "seal", "skill", "asked", "life", "ideal", "flags", "side", "flake"
];
...
export default function Lesson2_2() {
  ...
  const handleKeyDown = (e) => {
    /* HANDLE POPUP COMPLETION MODAL REDIRECT */
    if (showPopup) {
      if (e.key === " ") {
        e.preventDefault();
        navigate("/lesson2/2.3");
      }
      return;
    }
    ...
    if (key.length === 1 || key === " ") {
      ...
      // Strict case-sensitive matching verification against the lowercase words
      ...
      if (isCorrect) {
        ...
        /* WRONG KEY EVENT DISPATCHER */
        setWrongCount((prev) => prev + 1);
        setStatus("Wrong ❌");

        let displayInstruction = typedChar === " " ? "SPACE" : typedChar;

        // Validation check for active capitalization errors (caps lock / shift) against lowercase targets
        if (
          typedChar !== " " && 
          expectedChar !== " " && 
  ...
  /* PERFORMANCE ANALYTICS CALCULATORS */
  ...
  const nextTargetChar = currentWord[typed.length];
  ...
  return (
    ...
    >
      {/* Header Area Back Actions Panel */}
      ...
      <div className="w-full max-w-[1400px] mx-auto grid grid-cols-[180px_1fr_240px] gap-8 items-start">
        ...
        {/* CENTER COLUMN: HARDWARE MAP & WORD EXERCISE CONTAINER */}
        <div className="flex flex-col gap-6 w-full">
          <div className="grid grid-cols-[1fr_minmax(400px,460px)] gap-6 items-center bg-[#111827] border border-[#2b3245] rounded-[32px] p-6 shadow-xl">
                ...
                {topKeys.map((key) => (
                  <div
                    ...
                    className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl font-bold transition-all border shadow-md
                      ${
                        nextTargetChar?.toUpperCase() === key
            ...
            <div className="bg-[#050816] p-3 rounded-2xl border border-[#222] flex flex-col items-center">
              <img
                src={lessonKeyboard}
                alt="keyboard layout map display selector"
          ...
          {/* Central Workspace Canvas Wrapper Box */}
          <div className="bg-[#02040f] border border-[#1b2236] rounded-[32px] shadow-2xl relative flex flex-col items-center justify-center" style={{ padding: "24px", height: "340px" }}>
            ...
            {/* CURRENT WORD DISPLAY STRING TARGET */}
            <div className="text-6xl font-black tracking-[6px] mb-6 text-purple-500 font-mono">
              {currentWord}
            </div>

            {/* LIVE DISPLAY RENDERING FRAME BUFFER FOR INPUT CHARS */}
        ...
        {/* RIGHT COLUMN: RUNTIME STRING MONITOR & CASE ERROR PANEL */}
        <div className="flex flex-col gap-4 w-full">
          ...
          {/* DYNAMIC CASE ERROR WARNING REPORT CONTAINER */}
      ...
      {/* POPUP COMPLETION MODAL COMPONENT */}
      {showPopup && (
