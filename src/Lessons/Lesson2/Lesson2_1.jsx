...
// Image Assets
import lessonKeyboard from "../../assets/lesson2.png";

const keyPairs = [
  "ed", "ik", "ke", "ek", "di", "ie", "de", "ki", "id", "kd", "ei", "dk",
];
...
export default function Lesson2_1() {
  ...
  const generatePair = () => {
    const randomPair = keyPairs[Math.floor(Math.random() * keyPairs.length)];
  ...
  const handleKeyDown = (e) => {
    ...
    if (key.length === 1 || key === " ") {
      ...
      // FIXED: Directly compare typed input character values to support native lowercase entries
      ...
      if (isCorrect) {
        ...
        /* WRONG KEY PRESSED LOGIC */
        setWrongCount((prev) => prev + 1);
        setStatus("Wrong ❌");

        let displayInstruction = typedChar === " " ? "SPACE" : typedChar;

        // Validation check for active capitalization errors against lowercase targets
        if (
          typedChar !== " " && 
          expectedChar !== " " && 
  ...
  const nextTargetChar = currentPair[typed.length];
  const topKeys = ["E", "D", "I", "K"];
        ...
        <div className="flex flex-col gap-6 w-full">
          <div className="grid grid-cols-[1fr_minmax(400px,460px)] gap-6 items-center bg-[#111827] border border-[#2b3245] rounded-[32px] p-6 shadow-xl">
            ...
            <div className="bg-[#050816] p-3 rounded-2xl border border-[#222] flex flex-col items-center">
              ...
              />
              <span className="text-[11px] text-gray-500 mt-1 block tracking-wider font-mono">lesson2.png</span>
            </div>
          </div>
