...
// Image Assets
import lessonKeyboard from "../../assets/lesson8.1.png";

// Hand territories for tracking targets
const leftHandKeys = ["v"];
const rightHandKeys = ["n"];

// Generates practice streams containing ONLY 'v', 'n', and spaces
const generateWordParagraph = () => {
  const templates = [
    "v n v n v n n v v n v n v v n v n v n n v v n v n n v n v n v",
    "h h g g h g h g h g h g g h g h g h g h g h h g g h g h g g h", // Kept template count structural
    "n n v v n v n v n v n v v n v n v n v n v n n v v n v n v v n",
    "v n v n v v n v n v n n v n v n v n n v n v n v n v n v n v v",
    "v n v v n n v v n v n n v n v n v n v n v n v v n n v v n n v",
    "n v n v n n v v n v n v n v v n v n n v n v n v n v v n n v v"
  ];

  // Adjusting template indices to strictly match pure v/n sequences
  const pureTemplates = [
    templates[0],
    templates[2],
    templates[3],
  ...
  const handleKeyDown = (e) => {
    ...
    if (key.length === 1 || key === " ") {
      ...
      if (isCorrect) {
        ...
        if (expectedChar === "v") {
          displayInstruction = "DROP LEFT INDEX FINGER DOWN-RIGHT FOR V";
        } else if (expectedChar === "n") {
          displayInstruction = "DROP RIGHT INDEX FINGER DOWN-LEFT FOR N";
        }
  ...
  return (
    ...
    >
      ...
      <h1 className="text-4xl font-extrabold tracking-wide text-center w-full max-w-[1440px] mx-auto mb-12">
        Lesson 8.1 — Isolated Finger Drill (v and n Keys)
      </h1>
      ...
      <div className="w-full max-w-[1440px] mx-auto grid grid-cols-1 lg:grid-cols-[200px_1fr_420px] gap-10 items-start">
          ...
          <div className="bg-[#111827] border border-[#2b3245] border-l-[10px] border-l-purple-500 rounded-2xl px-10 py-8 shadow-2xl flex flex-col w-full">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-xl font-extrabold tracking-wide text-purple-400">
                Fingering Guide for V & N
              </h2>
            </div>
            <p className="text-md leading-relaxed text-gray-300 font-medium">
              Drop your <span className="text-amber-400 font-bold">Left Index Finger</span> down and slightly right from <span className="underline">F</span> to hit <span className="text-amber-400 font-bold">V</span>. Drop your <span className="text-[#00E676] font-bold">Right Index Finger</span> down and slightly left from <span className="underline">J</span> to hit <span className="text-[#00E676] font-bold">N</span>. Keep your wrist positions stable.
            </p>
          </div>
        ...
        <div className="flex flex-col gap-6 w-full">
          ...
          <div className="bg-[#111827] border border-[#2b3245] rounded-3xl p-8 shadow-xl flex flex-col items-center text-center justify-center min-h-[180px]">
            ...
            <h3 className={`text-6xl font-black font-mono ${nextTargetChar === " " ? "text-gray-500" : nextTargetChar === "v" ? "text-amber-400" : "text-[#00E676]"}`}>
