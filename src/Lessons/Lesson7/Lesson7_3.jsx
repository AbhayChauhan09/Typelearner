...
// Hand territories for advanced cross-coordination Shift tracking
const leftHandKeys = ["a", "s", "d", "f", "e", "r", "t", "c", "g", "A", "S", "D", "F", "E", "R", "T", "C", "G"];
const rightHandKeys = ["j", "k", "l", ";", "u", "i", "o", ",", "h", "'", "\"", "J", "K", "L", ":", "U", "I", "O", "<", "H"];

// Generates capital-heavy structural paragraph configurations blending home, top, bottom, and pinky rows
const generateWordParagraph = () => {
  const paragraphTemplates = [
    "Jared said, 'Check the core code sequence right here.' It is Clear, Fast, and Safe to Use. Call Her to Test its Total Logic or It Fails.",
    "Cats, Dogs, and Horses are at the Old side Field. To Care for Them is a Real Duty, Not a Casual Task. Create a Safe Space Right Now.",
    "Eli checks the Data File, Corrects Three Total Lines, and Runs It. If It Succeeds, Save It on a Secure Disk; Do Not Delay the Test.",
    "Cool Air, Clear Skies, and Calm Lakes make a Great Area. Let Us Sail Far Out, Past the Old Dock, to See the Entire Coastal View.",
    "Cook the Regular Rice, Cut the Fresh Fruit, and Set the Table. It is Time to Eat, so Call Your Close Friends to Come Sit Down."
  ];
...
export default function Lesson7_3() {
  ...
  const handleKeyDown = (e) => {
    ...
    if (key.length === 1 || key === " ") {
      ...
      if (isCorrect) {
        ...
        // Custom validation prompts matching comprehensive layout coordinates
        if (
          typedChar !== " " && 
          expectedChar !== " " && 
          expectedChar !== ";" &&
  ...
  // Evaluates shift state demands for indicators dynamically
  const dynamicShiftTarget = nextTargetChar !== "—" && nextTargetChar !== " " 
    ? (nextTargetChar === "\"" || nextTargetChar === ":" || nextTargetChar === nextTargetChar.toUpperCase() && nextTargetChar !== ";" && nextTargetChar !== "," && nextTargetChar !== "'")
    : false;
  ...
  return (
    ...
    >
      ...
      <h1 className="text-4xl font-extrabold tracking-wide text-center w-full max-w-[1440px] mx-auto mb-12">
        Lesson 7.3 — Full Array Pool Integration Review
      </h1>
        ...
        <div className="flex flex-col gap-10 w-full">
          ...
          <div className="bg-[#111827] border border-[#2b3245] border-l-[10px] border-l-purple-500 rounded-2xl px-10 py-8 shadow-2xl flex flex-col w-full">
            ...
            <p className="text-md leading-relaxed text-gray-300 font-medium">
              This module brings together <span className="text-purple-400 font-bold">all taught zones</span>. Monitor your lateral finger stretches for <span className="underline">G</span> and <span className="underline">H</span>, lock your middle finger paths down onto bottom-row commas and <span className="underline">C</span>, and control your Shift matching rules closely.
            </p>
          </div>
